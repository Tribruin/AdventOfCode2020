...
class Boot_Code:
    def __init__(self, input_file) -> None:
        ...
        with open(INPUT_FILE, mode="r") as f:
            ...
            self.code = [
                {"op": line.split()[0], "arg": int(line.split()[1]), "run": False}
                for line in code_lines
            ]
        self.orig_code = [code.copy() for code in self.code]

    def _reset_code(self):
        self.code = [code.copy() for code in self.orig_code]
        self.instruction = 0
        self.acc = 0
    ...
    def exec_code(self):
        while self.instruction < len(self.code):
            ...
            op, arg, prev_exec = (
                self.code[self.instruction]["op"],
                self.code[self.instruction]["arg"],
                self.code[self.instruction]["run"],
            )
            if prev_exec:
                return False

            if op == "nop":
                self.code[self.instruction]["run"] = True
                self.instruction += 1
    ...
    def run_code_check(self):
        ...
        while True:
            self._reset_code()
            op = self.code[test_cycle]["op"]

            if op in ["nop", "jmp"]:
                if op == "nop":
                    self.code[test_cycle]["op"] = "jmp"
                else:
                    self.code[test_cycle]["op"] = "nop"
